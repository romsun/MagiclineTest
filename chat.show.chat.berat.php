...
function openPic(message)
{
window.open("http://citifren.com/chat/images/"+message+"","","");
}
...
if($username4auth){ echo "$username4auth"; $cookiename = "citifren"; $value = "$username4auth"; setcookie($cookiename,$value, time()+3600*24); }
	...
	if($phone !== "$me"){ $divcolor = "callout right"; $divcounter = 1; 
		...
		$m = $message;
		$message = "<img onClick='javascript:offkan(); openPic($m)' width=50px src=images/$message>";
        	...
        	echo ""
        	.""
        	."<a href=images/$m target=luar>Gambar</a> $message"
        	."</div>"
        	."</td></tr></table>"
        	."";
